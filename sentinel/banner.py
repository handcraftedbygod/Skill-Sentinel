"""ASCII-art hero banner, mascot, and welcome-screen rendering for the CLI.

Split out of cli.py: this is pure presentation, colocating it with the actual
scan orchestration made that file harder to review for the logic that matters.
"""
...
def _write_line(text: str, stream) -> None:
    # The hero/welcome screen always contains block-art and box-drawing
    # characters, and sys.stdout/stderr's encoding defaults to the console's
    # legacy codepage (e.g. cp1252 on Windows) when not attached to a real
    # interactive terminal — plain print() would crash outright rather than
    # just render imperfectly.
